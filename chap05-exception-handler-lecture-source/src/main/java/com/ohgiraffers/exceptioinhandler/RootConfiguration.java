...
public class RootConfiguration {
    ...
    public SimpleMappingExceptionResolver simpleMappingExceptionResolver(){
        ...
        props.setProperty("java.lang.NullPointException", "error/nullPointer");
        props.setProperty("MemberRegistException", "error/memberRegist");

        /* 설명. 전제 예외 관련되어 SimpleMappingExceptionresolve에 설정하기 */
        /* 설명 1. 예외에 따른 페이지 설정한 것 활용 */
        exceptionResolver.setExceptionMappings(props);

        /* 설명 2. 그 외 나머지 예외에 대한 defalut 페이지 설정 */
        exceptionResolver.setDefaultErrorView("error/default");

        /* 설명 3. 예외 메세지를 뽑기 위한 Attribute key 값(화면에서 예외 메세지 확인 지 사용할 값) */
